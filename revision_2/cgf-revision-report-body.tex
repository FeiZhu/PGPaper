...
\noindent{Q: }\emph{Explicit comparisons to other methods (i.e., tet-based fracture following O'Brien's work and MLS fracture following M$\ddot{u}$ller's or Pauly's work) is missing. Some simple scenarios showing explicit comparisons would definitely highlight the issues that other methods have trouble with and that are apparently simpler with the peridynamics method}
